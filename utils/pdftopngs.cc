...
static const ArgDesc argDesc[] = { { "-f", argInt, &firstPage, 0, "first page to print" },
                                   ...
                                   { "-ppm", argFlag, &ppm, 0, "generate a PPM file" },
                                   ...
                                   { "-singletiff", argFlag, &singleTiff, 0, "generate a single TIFF file" },
...
static void savePageSlice(PDFDoc *doc, SplashOutputDev *splashOut, int pg, int x, int y, int w, int h, double pg_w, double pg_h, char *ppmFile, bool appendTiff)
{
    if (w == 0)
        w = (int)ceil(pg_w);
    if (h == 0)
